...
def enrich_building_heights_from_points(
    ...
    for building_index, building_geometry in buildings_metric.geometry.items():
        ...
        if not inside.empty:
            if len(inside) == 1:
                selected_height = float(inside[HEIGHT_COLUMN].iloc[0])
                source = "INE"
            else:
                centroid = building_geometry.centroid
                closest_point_index = inside.distance(centroid).idxmin()
                selected_height = float(points_metric.loc[closest_point_index, HEIGHT_COLUMN])
                source = "INE"
            selected_heights[building_index] = selected_height
            selected_references[building_index] = source
            direct_match_heights.append(selected_height)
            direct_matches += 1

    area_median_height = None
    if direct_match_heights:
        area_median_height = float(np.median(np.array(direct_match_heights, dtype=float)))

    # Stage 2: 2-nearest average fallback.
    # Stage 3: if stage 2 fails, use median of direct INE matches in the same area/run.
    for building_index, building_geometry in buildings_metric.geometry.items():
        if building_index in selected_heights:
            continue
        distances = points_metric.distance(building_geometry)
        nearest_two = distances.nsmallest(2)
        if len(nearest_two) < 2:
            if area_median_height is not None:
                selected_heights[building_index] = area_median_height
    ...
    unchanged = len(enriched) - direct_matches - two_nearest_matches - median_matches
    print(
        f"INE height enrichment applied: {direct_matches} direct matches, "
        f"{two_nearest_matches} 2-nearest matches, "
        f"{median_matches} area-median matches, "
...
def enrich_building_heights_from_gpkg(
    ...
    search_margin_m = max(fallback_max_distance_m, 1.0)
