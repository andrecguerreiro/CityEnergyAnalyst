...
for img_path, r in zip(all_images, results):
    if img_path.name in golden_paths:
        continue
    img_path_str = str(img_path)
    out_file_prediction = output_predictions_path / (Path(img_path).stem + ".txt")
    if not out_file_prediction.exists():  #make_predictions:
        new_txt = t.cat(
    ...
    button_labels = ["No Modification","Mark very good", "Slight Manual Labor", "Very wrong"]
    callbacks = [on_keep_old, on_old_to_golden, on_manual_review, on_keep_new, on_new_to_golden]
