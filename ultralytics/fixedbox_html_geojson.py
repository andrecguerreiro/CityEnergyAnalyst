...
def retrieve_prediction_list(satellite_image, top_left_corner, res,
    ...
    for i in range(results.boxes.shape[0]):
        ...
        b.convert_tensor_prediction_to_building(
            ...
            roof_prediction=np.asarray(sigmoid(results.roof.data[i, :])),
...
def get_osm_buildings_cached(top_left, bottom_right):
    ...
    if os.path.exists(cache_path):
        print(f"[cache] Loading OSM data from cache: {cache_path}")
...
def import_cea_polygon_to_zone():
    """Import CEA's polygon_to_zone with a robust path fallback."""
    try:
        from cea.datamanagement.zone_helper import polygon_to_zone
    except ModuleNotFoundError:
        repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
        if repo_root not in sys.path:
            sys.path.insert(0, repo_root)
        from cea.datamanagement.zone_helper import polygon_to_zone
    return polygon_to_zone


def build_cea_ordered_buildings_geojson(polygon_ring_lon_lat):
    """
    Build OSM-derived building footprints using CEA zone-helper ordering and naming.
    Returns a FeatureCollection where each feature has `properties.cea_name`.
    """
    ...
    site_poly = Polygon(ring)
    poly_gdf = gpd.GeoDataFrame([{"geometry": site_poly}], crs="EPSG:4326")

    polygon_to_zone = import_cea_polygon_to_zone()
    zone_df = polygon_to_zone(
        ...
        include_building_parts=True,
        poly=poly_gdf,
    )

    if zone_df.crs is None:
        zone_df = zone_df.set_crs("EPSG:4326")
    else:
    ...
    for _, row in zone_df.iterrows():
        ...
        coords = [[float(x), float(y)] for x, y, *_ in geom.exterior.coords]
        features.append(
            {
                "type": "Feature",
...
def export_roof_surfaces_geojson(
    ...
    n_intersections = 0
    n_skipped_not_enough_points = 0
    n_skipped_not_polygon = 0

    def ordered_ring_from_points(points_3d):
        """
        Order an unordered 3D point cloud lying on a plane into a closed ring.
        """
        if points_3d.shape[0] < 3:
            return None

        # Remove duplicates with a stable precision.
        ...
        # Principal axes on the plane via SVD.
    ...
    geojson = {
        ...
        "crs": {
            "type": "name",
            "properties": {"name": "urn:ogc:def:crs:EPSG::32629"},
        },
        "features": features,
    }
    ...
    print(
        "Roof export debug: "
        f"matched_buildings={n_entries}, intersections_seen={n_intersections}, "
        f"skipped_not_enough_points={n_skipped_not_enough_points}, "
        f"skipped_not_polygon={n_skipped_not_polygon}"
    )
    return geojson
...
def build_map_html(osm_geojson, predicted_buildings, satellite_image,
    ...
    osm = folium.FeatureGroup(name="Zone Buildings")
    folium.GeoJson(osm_geojson,
        style_function=lambda _: {"fillColor":"#3388ff","color":"#1a55cc",
                                   "weight":2,"fillOpacity":0.3},
        tooltip=folium.GeoJsonTooltip(fields=["cea_name","building"],
                                       aliases=["CEA ID","Type"], localize=True)
    ).add_to(osm)
    osm.add_to(m)
    ...
    if matched_data:
        ...
        for entry in matched_data:
            ...
            cea_name = entry.get("cea_name", "?")
            print(f"Building {cea_name}: {len(lines_world)} topology line(s)")
            for line in lines_world:
                ...
                folium.PolyLine(
                    ...
                    tooltip=f"Building {cea_name} - roof line",
                ).add_to(topo_layer)
                n_lines_total += 1
...
def prompt_polygon_input_text():
    """
    Prompt for polygon input when no CLI input is provided.
    Accepts either a file path or pasted GeoJSON (single or multi-line).
    """
    print(
        "\nNo polygon input argument was provided.\n"
        "Paste GeoJSON text (FeatureCollection/Feature/Polygon), or type a .geojson file path.\n"
        "If pasting multi-line JSON, press Enter on an empty line to finish."
    )
    first_line = input("> ").strip()
    if not first_line:
        return ""

    # If this is a file path, load file content directly.
    if os.path.isfile(first_line):
        with open(first_line, "r", encoding="utf-8") as f:
            return f.read()

    # If this looks like JSON, allow multiline paste.
    ...
    # Fallback: return raw line and let JSON parser produce clear error.
    return first_line


def parse_polygon_from_args():
    default_bbox = {
  "north": 38.70862547788349,
  "south": 38.708020311066804,
  "east": -9.137582336508132,
  "west": -9.138313538505741
}



    parser = argparse.ArgumentParser(
        description="Run roof inference and export roof surfaces to GeoJSON."
    )
    parser.add_argument(
        "--polygon-json",
        nargs="+",
        default=None,
        help=(
            "Polygon payload as JSON. Accepted: FeatureCollection, Feature, "
            "Polygon, or raw [[lon,lat],...] ring."
        ),
    )
    parser.add_argument(
        "--polygon-geojson",
        type=str,
        default=None,
        help="GeoJSON text as one argument (FeatureCollection, Feature, Polygon, MultiPolygon, or LineString).",
    )
    parser.add_argument(
        "--polygon-file",
        type=str,
        default=None,
        help="Path to a JSON/GeoJSON file (FeatureCollection, Feature, or Polygon).",
    )
    parser.add_argument(
        "--polygon-stdin",
        action="store_true",
        help="Read polygon GeoJSON from stdin (pipe or redirected file).",
    )
    args = parser.parse_args()

    polygon_text = None
    if args.polygon_file:
        with open(args.polygon_file, "r", encoding="utf-8") as f:
            polygon_text = f.read()
    elif args.polygon_geojson:
        polygon_text = args.polygon_geojson
    elif args.polygon_json:
        # Support unquoted JSON passed as multiple shell tokens.
        polygon_text = " ".join(args.polygon_json)
    elif args.polygon_stdin:
        polygon_text = sys.stdin.read()
        if not polygon_text.strip():
            raise ValueError("No input received from stdin for --polygon-stdin.")
    else:
        polygon_text = prompt_polygon_input_text()
        if not polygon_text.strip():
            print("No input provided. Falling back to built-in default bbox.")
            return default_bbox, default_polygon_ring_from_bbox(default_bbox)
    ...
    try:
        ...
        raise ValueError(
            "Invalid JSON for polygon input. Use --polygon-file, --polygon-geojson, "
            "--polygon-json, or --polygon-stdin with valid GeoJSON."
        ) from exc
    coords = None
    ...
    if not coords or len(coords) < 3:
        raise ValueError("Invalid polygon-json. Provide a valid Polygon with at least 3 coordinates.")
...
model = CachedModel()
bbox, polygon_ring_lon_lat = parse_polygon_from_args()
...
print(f"[info] Area: {area_m2/1e6:.4f} km2")
print("[ok] EPSG:4326 -> EPSG:3763 done")

print("Connecting to WMTS...")
print("[info] Connecting to DGT WMTS satellite service...")
...
print(f"[ok] Service ready - {total_tiles} tile(s) to download")
...
print(f"[ok] Satellite image ready ({w}x{h} px, {res:.3f} m/px)")

print("Fetching CEA-ordered buildings...")
geojson = build_cea_ordered_buildings_geojson(polygon_ring_lon_lat)
print(f"[ok] CEA ordering done - {len(geojson['features'])} footprint(s)")


building_threshold = 0.05
overlap_threshold = 0.1
print("Running YOLO inference...")
print(f"[info] Running detector (conf>={building_threshold:.2f}, iou<={overlap_threshold:.2f})...")
...
print(f"[ok] {len(buildings)} building(s), {total_planes} roof plane(s)")

# here is where we need to assing all rooftop predictions to the CEA-ordered buildings
iou_mat = compute_iou_matrix(geojson['features'], buildings)
matched_data = []
 
for i, osm_feat in enumerate(geojson['features']):
    cea_name = str(osm_feat["properties"].get("cea_name", "")).strip() or f"B{i + 1000}"
    ...
    if best_iou > 0.3:
        ...
        print(f"[ASSOC] Building {cea_name} -> Pred {best_match_idx} | code={code} | faces={generated_faces}")
        matched_data.append({
            "cea_name": cea_name,
            "footprint": outline,
        ...
        print(f"[SKIP] Building {cea_name}: rejected (IoU <= 0.3)")

print(f"Matched buildings for topology conversion: {len(matched_data)}")

# Export roof surfaces to GeoJSON (one feature per roof face tile)
...
print("[ok] Map saved successfully to: building_map.html")
...
def render_3d_tiles_old(matched_data, base_height=10.0, max_roof_extension=20.0):
    ...
    for entry in matched_data:
        ...
        building_id = entry.get("cea_name", "Unknown")
...
def render_3d_tiles(matched_data):
    ...
    for entry in matched_data:
        ...
        if not intersections:
            print(f"[warn] Skipping building {building_id} - no intersection tiles found.")
            continue
    ...
    print(f"[info] Rendering {buildings_rendered} buildings in PyVista...")

    pl.add_axes()
    # Since these are in real world coordinates (EPSG:3763), the grid will reflect actual map coordinates
    pl.show_grid(xtitle="X (m - EPSG:3763)", ytitle="Y (m - EPSG:3763)", ztitle="Z (m)")
    pl.show()
